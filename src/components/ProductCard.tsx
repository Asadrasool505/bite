    ...
    <Link href={`/product/${product?.id}`} className="block h-full group">
      <div className="relative rounded-xl overflow-hidden shadow-2xl transition-all duration-300 border border-white/5 bg-white/5 backdrop-blur-sm h-full group-hover:-translate-y-1 group-hover:border-yellow-500/50 group-hover:shadow-[0_0_15px_rgba(250,204,21,0.2)] flex flex-col">
          ...
          <button
            ...
            className={`absolute top-3 left-3 z-10 p-2 rounded-full backdrop-blur-sm border transition-all hover:scale-110 cursor-pointer ${
              inCompare 
                ...
                : "bg-[#0A1128]/80 border-white/10 text-gray-400 hover:border-yellow-500/50 hover:text-yellow-400"
            }`}
        ...
        <div className="p-6 md:p-8 flex-1 flex flex-col">
          <h3 className="text-lg md:text-xl font-bold mb-1 transition-colors text-gray-200 group-hover:text-yellow-400">
            {product?.name}
          </h3>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span 
              ...
              className="text-[10px] text-gray-400 hover:text-yellow-400 hover:underline transition-colors font-medium cursor-pointer"
            >
              ({reviewsCount} {reviewsCount === 1 ? 'Review' : 'Reviews'})
            </span>
          </div>

          <div className="text-xs md:text-sm text-gray-400 mb-6 line-clamp-3 font-light" dangerouslySetInnerHTML={{ __html: product?.description || "" }} />
          
          <div className="mt-auto pt-6 border-t border-yellow-500/10">
            <p className="text-[10px] font-bold uppercase tracking-widest mb-3 text-yellow-500">{t("technical_specs")}</p>
            <ul className="text-xs text-gray-400 space-y-2 mb-6">
              <li><strong className="text-gray-300 font-bold">Handle:</strong> {product?.technical_specifications?.handle || "Ergonomic Offset"}</li>
              <li><strong className="text-gray-300 font-bold">Edge:</strong> {product?.technical_specifications?.edge || "Semi-Convex"}</li>
            </ul>
            <button 
              onClick={(e) => {
                e.preventDefault();
