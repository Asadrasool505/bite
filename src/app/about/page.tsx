...
function GoldRule() {
  return (
    <div
      ...
      style={{ background: "linear-gradient(to right, #D4AF37, rgba(212,175,55,0.2))" }}
    />
  );
}
...
export default function AboutPage() {
  return (
    <div className="w-full" style={{ background: "#050814" }}>
      ...
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#050814]/50 via-[#0A1128]/75 to-[#050814]" />
        <div className="absolute inset-0 bg-[#0A1128]/40" />

        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          <SectionEyebrow>Est. Sialkot · Pakistan</SectionEyebrow>
          <h1 className="text-5xl md:text-7xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-600 leading-tight tracking-wide uppercase mb-6">
      ...
      <div
        ...
        style={{ background: "linear-gradient(90deg, #080f2a, #0d1f4a, #080f2a)" }}
      >
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {stats.map((s) => (
            <div key={s.label}>
              <p className="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-600 mb-1">
                {s.value}
              </p>
              <p className="text-gray-500 text-xs uppercase tracking-widest font-light">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ══════════════════════════════════════
          2. PHILOSOPHY — The Bite Standard
      ══════════════════════════════════════ */}
      <section className="py-28 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative group">
            <div className="rounded-3xl overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.7)] border border-white/5">
              <img
            ...
            {/* Floating gold badge */}
            <div className="absolute -bottom-5 -right-5 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-2xl px-5 py-4 shadow-2xl text-[#0A1128]">
          ...
          <div className="flex flex-col gap-6">
            ...
            <p className="text-gray-300 text-base md:text-lg font-light leading-relaxed">
              We don't just make tools — we forge <em className="text-white font-medium not-italic">extensions of a groomer's hand</em>. Every Bite Instruments shear is a collaboration between centuries of Sialkot metalwork knowledge and the uncompromising expectations of today's elite grooming professionals.
            </p>
            ...
            <div className="flex flex-col gap-3 mt-4">
              {["Japanese J2 & VG10 Steel exclusively", "11-stage quality inspection protocol", "Ergonomic offset handles — 60% less wrist fatigue", "Lifetime warranty on all instruments"].map((point) => (
                <div key={point} className="flex items-center gap-3">
                  ...
                  <p className="text-gray-300 text-sm font-light">{point}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ══════════════════════════════════════
          3. MANUFACTURING — 3 Pillars
      ══════════════════════════════════════ */}
      <section
        className="py-28 px-6"
        style={{ background: "linear-gradient(180deg, #080f24 0%, #0A1128 100%)" }}
      >
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <SectionEyebrow>Manufacturing Excellence</SectionEyebrow>
            <h2 className="text-4xl md:text-5xl font-extrabold text-white uppercase tracking-wider">
              Three Pillars of{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-600">
                Mastery
              </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pillars.map((p, i) => (
              <div
                key={p.title}
                className="group relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 hover:border-yellow-500/30 hover:-translate-y-2 transition-all duration-500 hover:shadow-[0_20px_60px_rgba(212,175,55,0.1)]"
              >
                {/* Pillar number */}
                <span className="absolute top-6 right-6 text-6xl font-black text-white/4 select-none pointer-events-none leading-none">
                  {String(i + 1).padStart(2, "0")}
                </span>

                {/* Icon */}
                <div className="w-14 h-14 rounded-2xl bg-yellow-400/10 border border-yellow-500/20 flex items-center justify-center text-yellow-400 mb-6">
                  {p.icon}
                </div>

                <h3 className="text-white text-xl font-bold tracking-wide mb-3">{p.title}</h3>
                <GoldRule />
                <p className="text-gray-400 text-sm font-light leading-relaxed mt-5">{p.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ══════════════════════════════════════
          4. GLOBAL REACH & MISSION
      ══════════════════════════════════════ */}
      <section className="py-28 px-6" style={{ background: "#050814" }}>
        <div className="max-w-7xl mx-auto">

          {/* Mission statement — full width pull quote */}
          <div className="text-center mb-20 max-w-4xl mx-auto">
            <SectionEyebrow>Our Mission</SectionEyebrow>
            <blockquote className="text-3xl md:text-4xl font-light text-white leading-relaxed italic tracking-wide">
              "To empower grooming professionals globally with the world's most{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-600 not-italic font-bold">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {regions.map((r) => (
              <div
                key={r.region}
                className="flex flex-col items-center gap-2 bg-white/5 border border-white/10 rounded-2xl p-5 text-center hover:border-yellow-500/30 transition-all duration-300"
              >
                <span className="text-4xl">{r.flag}</span>
                <p className="text-white text-xs font-bold tracking-wide">{r.region}</p>
                <p className="text-gray-600 text-[10px] font-light">{r.detail}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ══════════════════════════════════════
          5. CTA — Upgrade Your Salon
      ══════════════════════════════════════ */}
      <section
        className="relative py-28 px-6 overflow-hidden"
        style={{ background: "linear-gradient(135deg, #0d1f4a 0%, #080f2a 50%, #0A1128 100%)" }}
      >
        {/* Gold spotlight glow */}
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[300px] pointer-events-none"
          style={{ background: "radial-gradient(ellipse, rgba(212,175,55,0.08) 0%, transparent 70%)", filter: "blur(40px)" }}
        />

        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <SectionEyebrow>Take the Next Step</SectionEyebrow>
          <h2 className="text-4xl md:text-6xl font-extrabold text-white uppercase tracking-wider mb-6 leading-tight">
          ...
          <div className="flex flex-col sm:flex-row gap-5 justify-center">
            <Link
              href="/contact"
              className="px-10 py-4 rounded-full font-black uppercase tracking-widest text-sm text-[#0A1128] bg-gradient-to-r from-yellow-400 to-yellow-600 hover:shadow-[0_0_30px_rgba(212,175,55,0.5)] hover:-translate-y-1 transition-all duration-300"
            >
              Contact Us
            </Link>
            <Link
              href="/"
              className="px-10 py-4 rounded-full font-bold uppercase tracking-widest text-sm text-yellow-400 border border-yellow-500/50 hover:bg-yellow-500/10 hover:-translate-y-1 transition-all duration-300"
            >
              View Collection
            </Link>
          </div>
